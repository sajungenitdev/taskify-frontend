...
export default function AllDepartmentsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (deptResponse.data.success) {
        ...
        // Map users to departments with better ID comparison
        const departmentsWithMembers = depts.map((dept: any) => {
          // Ensure we have the correct department ID for comparison
          const deptId = dept._id?.toString() || dept.id?.toString() || '';
          ...
          const members = users.filter((u: any) => {
            ...
            if (typeof u.department === 'object' && u.department !== null) {
              userDeptId = u.department._id?.toString() || u.department.id?.toString() || '';
            } else if (typeof u.department === 'string') {
              userDeptId = u.department;
            } else if (typeof u.department === 'object' && u.department._id) {
              userDeptId = u.department._id.toString();
            }
  ...
  // Optional: Keep this for tab focus refresh if needed
  useEffect(() => {
    const handleFocus = () => {
