...
function TenderManageContent() {
  ...
  const handleApproveForParticipation = async (
    tender: Tender,
    ...
    try {
      ...
      await tenderApi.changeStage(tender._id, "active", "Eligibility approved");
                ...
                {selected && (
                  <TenderDetailReview
                    ...
                    data={{
                      ...
                      eligibility: (() => {
                        const saved = eligibilityResults[selected._id];
                        if (saved && saved.length > 0) {
                          const meets = saved
                            .filter((r) => r.match === "Meets")
                            .map((r) => r.requirement)
                            .filter(Boolean);
                          if (meets.length > 0) return meets.join(",\n");
                        }
                        return toReviewDetail(selected).eligibility ?? "";
                      })(),
                    }}
                    onCheckEligibility={() => setEligibilityTender(selected)}
                    onApprove={() => handleApproveForParticipation(selected)}
                    onOpenChecklist={() => setChecklistTender(selected)}
                    onUploadAttachment={async (file) => {
                      const loadingId = toast.loading(`Uploading ${file.name}...`);
                      try {
                        await tenderApi.uploadAttachment(selected._id, file);
                    ...
                    onDeleteAttachment={async (attachmentId) => {
                      try {
                        await tenderApi.deleteAttachment(selected._id, attachmentId);
                    ...
                    onUploadAdvertisement={async (file) => {
                      const loadingId = toast.loading(`Uploading ${file.name}...`);
                      try {
                        const res = await tenderApi.uploadAdvertisement(selected._id, file);
                        toast.success("Advertisement uploaded", { id: loadingId });
                        refetch();
                        return res;
                      } catch (e) {
                        toast.error((e as Error).message || "Upload failed", { id: loadingId });
                    ...
                    onDeleteAdvertisement={async () => {
                      try {
                        await tenderApi.update(selected._id, {
                          advertisementFile: "",
                          advertisementUrl: "",
                        });
                        ...
                        toast.error((e as Error).message || "Failed to remove advertisement");
                        throw e;
                      }
                    }}
                  />
      ...
      <EligibilityCheckModal
        ...
        onApprove={(requirements) => {
          if (eligibilityTender) {
            ...
            handleApproveForParticipation(eligibilityTender, requirements);
          }
        }}
        onDecline={() => {
          if (eligibilityTender) {
