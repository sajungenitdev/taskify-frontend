...
export default function TaskAssistant({
  ...
  return (
    <div className="bg-linear-to-br from-indigo-500/10 to-purple-500/10 rounded-xl border border-indigo-500/20 overflow-hidden">
      <button
          ...
          {suggestions.priority && (
            <div className="bg-slate-800/50 rounded-lg p-2">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${getPriorityColor(suggestions.priority.priority)}`}
                >
                  {suggestions.priority.priority.toUpperCase()}
                </span>
                <span className="text-xs text-slate-500">
                  {suggestions.priority.reason}
                </span>
              </div>
            </div>
          )}
            ...
            <div className="bg-slate-800/50 rounded-lg p-2">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-sm font-bold text-white">
                  {suggestions.duration.hours} hours
                </span>
                <span className="text-xs text-slate-500">
                  {suggestions.duration.explanation}
                </span>
              </div>
            </div>
          )}
          ...
          {suggestions.subtasks && suggestions.subtasks.length > 0 && (
            <div className="bg-slate-800/50 rounded-lg p-2">
              ...
              <ul className="space-y-1">
                {suggestions.subtasks.map((subtask: Subtask, index: number) => (
                  <li
                    key={index}
