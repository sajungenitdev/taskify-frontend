...
const formatCurrencyWithCode = (amount: number, code?: string): string => {
    const upper = (code || "BDT").toUpperCase();
...
const ExpenseRow = memo(function ExpenseRow({
    ...
    // 👇 Admins can edit/delete/select any row; employees only pending
    const canDelete = isPending || isApprover;
    const canEdit = isPending || isApprover;
    ...
    const currencyCode = (expense.currency || "BDT").toUpperCase();

    return (
        <tr
            className={`transition-colors duration-150 group ${isSelected ? "bg-slate-50" : "hover:bg-slate-50/70"
                }`}
        >
            {/* Checkbox */}
            ...
            <td className="px-5 py-3.5 text-right font-mono whitespace-nowrap">
                <span className="text-sm font-bold text-slate-950">
                    {formatCurrencyWithCode(expense.amount, currencyCode)}
                </span>
                ...
                <div className="flex items-center justify-end gap-1.5">
                    ...
                    {canEdit && (
                        <button
                            ...
                            className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-colors cursor-pointer"
                            title={isApprover && !isPending ? "Edit (admin override)" : "Edit Claim"}
                        >
                    ...
                    {canDelete && (
                        <button
                            ...
                            title={isApprover && !isPending ? "Delete (admin override)" : "Delete Claim"}
                        >
...
export default function MyExpensesTable({
    ...
    const filteredExpenses = useMemo(() => {
        ...
        return expenses.filter((item) => {
            ...
            const currencyMatch = (item.currency || "")
                .toLowerCase()
                .includes(query);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-2xs">
                <div className="relative flex-1 max-w-md">
                    ...
                    <input
                        ...
                        placeholder="Search claims, submitters, locations, or currency..."
