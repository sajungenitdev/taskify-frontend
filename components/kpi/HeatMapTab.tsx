  ...
  if (!heatMapData || heatMapData.length === 0) {
    return (
      <div className="text-center py-12 bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200 shadow-sm">
        ...
        <h3 className="text-lg font-semibold text-gray-800 mb-2">No Heat Map Data</h3>
        <p className="text-gray-500">No employee data available for heat map visualization</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-700">Performance Heat Map</h3>
          ...
          <table className="w-full">
            <thead className="bg-gray-50/80 border-b border-gray-200">
              <tr>
                ...
                {components.map((comp) => (
                  <th key={comp.key} className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <div className="text-[10px]">{comp.label}</div>
                  </th>
                ))}
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Total</th>
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Level</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-4 py-3 sticky left-0 bg-white">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-800">{employee.employeeName}</p>
                          <p className="text-xs text-gray-400">{employee.department}</p>
                        </div>
                      </div>
                    </td>
                    {components.map((comp) => {
                      const value = employee[comp.key as keyof HeatMapData] as number;
                      return (
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-sm font-bold text-gray-800">{formatScore(employee.totalScore)}%</span>
                    </td>
                    <td className="px-4 py-3 text-center">
